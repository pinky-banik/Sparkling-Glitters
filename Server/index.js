...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.5f7tq.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run (){
    try{
            ...
            const database = client.db("sparkling-glitters");
            ...
            app.put("/updateOrderStatus", async (req, res) => {
                ...
                const result = await clientOrderCollection.updateOne(
                  { _id: ObjectId(id) },
                  {
                    $set: { status: status },
                  }
...
app.get('/', (req, res) =>{
    res.send('decorative lights is running');
});
